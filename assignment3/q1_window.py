...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        prefix = window_size * start
        suffix = window_size * end
        padded_sentence = [prefix] + sentence + [suffix]
        for index, label in enumerate(labels):
            window_features = padded_sentence[index : index + (2*window_size+1)]
            features = list(itertools.chain.from_iterable(window_features))
            windowed_data += [(features, label)]
    ### END YOUR CODE
