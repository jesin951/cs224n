...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    Return:
    cost -- cross entropy cost for the softmax word prediction {J}
    gradPred -- the gradient with respect to the predicted word {dv}
           vector
    grad -- the gradient with respect to all the other word {du}
           vectors
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    ### YOUR CODE HERE
    # variable names as in writeup; vectorized q := 'qs'
    ...
    cost = - np.log(p) - np.sum(np.log(qs))

    # gradPred
    gradPred = - (1-p) * u_o + (1-qs).dot(samples)
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    for word in contextWords:
        target = tokens[word]
        currentCost, currentGradIn, currentGradOut = \
            word2vecCostAndGradient(predicted, target, outputVectors, dataset)
        cost += currentCost
        gradIn[currentIndex] += currentGradIn
